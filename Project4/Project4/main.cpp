...
city* addCity(city *head, city *previous, string cityName)
{
    ...
    newCity->next = NULL;
    city *tmp = head;
    if (previous == head)//Adds cities to beginning of list
    {
    ...
    else if (previous == NULL)//Adds cities to end of list
    {
        previous = head;
...
city *searchNetwork(city *ptr, string cityName)
{
    ...
    while (ptr->name != cityName)//goes through list until it reaches the desired city
    {
        ptr = ptr->next;
    }
    // cout << ptr->name << endl;
...
city* deleteEntireNetwork(city *ptr)
{
    ...
    city *tmp = ptr;
    while (tmp != NULL)//goes through entire path and deletes each element
    {
        tmp = ptr->next;
...
void transmitMsg(city *head, string receiver, string message)
{
    ...
    city *sender = head;//keeps track of city with message
    city *next = head;//keeps track of the next city in list
    sender->message = message;//puts message into the sender
    while (sender->name != receiver)
    {
        sender->numberMessages = sender->numberMessages + 1;//increments number of messages through each city it passes through
        cout << sender->name << " [# messages passed: " << sender->numberMessages << "] received: " << sender->message << endl;
        next = sender->next;//finds next city in path
        next->message = sender->message;//send the message to the next city
        sender = sender->next;//makes the next city into the sender
    }
    if (sender->name == receiver)//output if city has recieved the message
    {
...
city* deleteCity(city *head, string cityName)
{
    ...
    city *previous = head;
    //cout << tmp->name << endl;
    while (tmp->next && tmp->name != cityName)//finds where city is in list
    {
        //cout << tmp->name << endl << previous->name << endl;
        previous = tmp;
        tmp = tmp->next;
    }
    if (tmp->name != cityName)//if city doesn't exist, outputs that it doesn't exist
    {
    ...
    {
        //cout << tmp->name << endl << previous->name << endl;
        previous->next = tmp->next;//sets the next of previous city to the city after the one to be deleted
        //cout << tmp->name << endl << previous->name << endl;
    }

    delete tmp;//deletes the city
    return head;
}
...
void printPath(city *ptr)
{
    ...
    {
        while (tmp->next)//goes through entire list
        {
            cout << tmp->name << " -> ";//prints out each city
            tmp = tmp->next;
        }
        cout << tmp->name << " -> NULL" << endl;//prints out the last city and shows that it points to null
    }
...
city* loadDefaultSetup(city *head)
{
    head = deleteEntireNetwork(head);
    //cout << head << endl;
    head = addCity(head, NULL, "Los Angeles");//each of these adds city to end of list
